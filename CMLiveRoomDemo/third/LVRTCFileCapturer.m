...
NSString *const kIMFileVideoCapturerErrorDomain = @"cn.liveme.LVRTCFileVideoCapturer";

typedef NS_ENUM(NSInteger, IMFileVideoCapturerErrorCode) {
    LVRTCFileVideoCapturerErrorCode_CapturerRunning = 2000,
    LVRTCFileVideoCapturerErrorCode_FileNotFound
};

typedef NS_ENUM(NSInteger, LVRTCFileVideoCapturerStatus) {
    LVRTCFileVideoCapturerStatusNotInitialized,
    LVRTCFileVideoCapturerStatusStarted,
    LVRTCFileVideoCapturerStatusStopped
};

@implementation LVRTCFileCapturer
{
    AVAssetReader *_reader;
    AVAssetReaderTrackOutput *_outTrack;
    LVRTCFileVideoCapturerStatus _status;
...
-(void)startCapturingFromFilePath:(NSString *)filePath onError:(LVRTCVideoCapturerErrorBlock)errorBlock{
    ...
    if (_status == LVRTCFileVideoCapturerStatusStarted) {
        NSError *error =
        [NSError errorWithDomain:kIMFileVideoCapturerErrorDomain
                            code:LVRTCFileVideoCapturerErrorCode_CapturerRunning
                        userInfo:@{NSUnderlyingErrorKey : @"Capturer has been started."}];
        
        errorBlock(error);
        return;
    } else {
        _status = LVRTCFileVideoCapturerStatusStarted;
    }
...
- (void)setupReaderOnError:(LVRTCVideoCapturerErrorBlock)errorBlock {
...
- (void)stopCapture {
    _status = LVRTCFileVideoCapturerStatusStopped;
}
...
- (void)readNextBuffer {
    if (_status == LVRTCFileVideoCapturerStatusStopped) {
