...
#import "LVRTCNSGLVideoView.h"

#import <AppKit/NSOpenGL.h>
#import <CoreVideo/CVDisplayLink.h>
#import <OpenGL/gl3.h>

#import "LVRTCDefaultShader.h"
#import "LVRTCI420TextureCache.h"
#import "base/RTCLogging.h"
#import "base/RTCVideoFrame.h"

@interface LVRTCNSGLVideoView ()
// |videoFrame| is set when we receive a frame from a worker thread and is read
// from the display link callback so atomicity is required.
@property(atomic, strong) RTCVideoFrame *videoFrame;
@property(atomic, strong) LVRTCI420TextureCache *i420TextureCache;
...
static CVReturn OnDisplayLinkFired(CVDisplayLinkRef displayLink,
  ...
  LVRTCNSGLVideoView *view = (__bridge LVRTCNSGLVideoView *)displayLinkContext;
  [view drawFrame];
  return kCVReturnSuccess;
}

@implementation LVRTCNSGLVideoView {
  CVDisplayLinkRef _displayLink;
  RTCVideoFrame *_lastDrawnFrame;
  id<LVRTCVideoViewShading> _shader;
}
...
- (instancetype)initWithFrame:(NSRect)frame pixelFormat:(NSOpenGLPixelFormat *)format {
  return [self initWithFrame:frame pixelFormat:format shader:[[LVRTCDefaultShader alloc] init]];
}

- (instancetype)initWithFrame:(NSRect)frame
                  pixelFormat:(NSOpenGLPixelFormat *)format
                       shader:(id<LVRTCVideoViewShading>)shader {
...
- (void)drawFrame {
  ...
  if (!self.i420TextureCache) {
    self.i420TextureCache = [[LVRTCI420TextureCache alloc] initWithContext:context];
  }
  LVRTCI420TextureCache *i420TextureCache = self.i420TextureCache;
  if (i420TextureCache) {
